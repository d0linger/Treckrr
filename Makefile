...
GOTEST := docker run --rm --network treckrr-itest -v "$(CURDIR):/src" -v treckrr-gomod:/go/pkg/mod -w /src -e GOTOOLCHAIN=go1.27.1 -e TEST_DATABASE_URL="$(TEST_DATABASE_URL)" $(IMG) sh -c
...
lint: ## golangci-lint
	docker run --rm -v "$(CURDIR):/src" -v treckrr-gomod:/go/pkg/mod -w /src -e GOTOOLCHAIN=go1.27.1 $(LINT) golangci-lint run ./...
