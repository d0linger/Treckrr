...
func (s *Server) handleRecurringCreate(w http.ResponseWriter, r *http.Request) {
	...
	if entry.Voided {
		s.setFlash(w, r, "error", "Stornierte Buchungen können nicht als Vorlage für wiederkehrende Buchungen verwendet werden.")
	...
	if s.tooLong(w, r, "Nächste Ausführung", nextRun, maxNameLen) {
		redirect(w, r, "/recurring")
		return
	}
