...
func TestRecurringCreate_NextRunLengthLimit(t *testing.T) {
	s := testRecurServer(t)

	// Oversized date payload (e.g. maxNameLen+1 characters)
	oversizedDate := strings.Repeat("x", maxNameLen+1)

	form := url.Values{}
	form.Set("next_run", oversizedDate)
	...
	if rr.Code != http.StatusSeeOther {
		t.Errorf("expected status SeeOther (%d), got %d", http.StatusSeeOther, rr.Code)
	}

	loc := rr.Header().Get("Location")
	if loc != "/recurring" {
		t.Errorf("expected redirect to /recurring, got %s", loc)
	}

	flashCookie := flashText(t, s, rr)
	if !strings.Contains(flashCookie, "Startdatum darf höchstens 100 Zeichen lang sein.") {
		t.Errorf("expected oversized next_run warning, got cookie: %q", flashCookie)
	}
}
