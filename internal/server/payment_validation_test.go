...
func (r *mockPaymentRows) Columns() []string {
	...
	case strings.Contains(r.query, "SELECT p.id"):
...
func TestHandlePaymentUpdateValidation(t *testing.T) {
	...
	t.Run("overly long paid_on rejected", func(t *testing.T) {
		...
		req := httptest.NewRequest(http.MethodPost, "/payments/1/update", strings.NewReader(form.Encode()))
		req.Header.Set("Content-Type", "application/x-www-form-urlencoded")
		req.SetPathValue("id", "1")
		rr := httptest.NewRecorder()
...
func TestHandleInstallmentAddValidation(t *testing.T) {
	...
	t.Run("overly long due_on rejected", func(t *testing.T) {
		longDate := strings.Repeat("2026-01-01", 5) + "X" // 51 chars
		form := url.Values{}
		form.Set("year_id", "1")
		...
		flashCookie := flashText(t, s, rr)
		if !strings.Contains(flashCookie, "Fällig am darf höchstens 50 Zeichen lang sein.") {
			t.Errorf("expected long due_on flash message, got cookie: %q", flashCookie)
		}
	})
}
