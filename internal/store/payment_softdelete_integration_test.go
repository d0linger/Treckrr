...
func TestPaymentSoftDeleteIntegration(t *testing.T) {
	...
	f := fixtures{Years: []int{4300 + pid%1000}, NeighborNames: []string{fmt.Sprintf("SD Nachbar %d", pid)}}
	purgeFixtures(t, ctx, pool, f)
	t.Cleanup(func() { purgeFixtures(t, ctx, pool, f) })

	baseID, err := st.CreateEmptyBase(ctx, 4300+pid%1000, "SD-Basis")
	if err != nil {
		t.Fatalf("base: %v", err)
	}
	yearID, err := st.CreateBillingYear(ctx, 4300+pid%1000, baseID, "SD-Jahr")
	if err != nil {
		t.Fatalf("year: %v", err)
	}
	nid, err := st.CreateNeighbor(ctx, fmt.Sprintf("SD Nachbar %d", pid), "")
	if err != nil {
