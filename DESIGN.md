...
Application canvases choose from quiet technical-paper compositions. They paint once and repaint only for resize or theme changes. The login worksheet is the explicit exception: its travelling vertical light band and synchronized machine highlights run continuously while visible, capped at 25 fps, without a five-second cutoff. The keyboard-accessible "Hintergrund pausieren" / "Hintergrund fortsetzen" control below the login card lets the user stop and restart this decorative motion. Pausing, hiding the page or leaving it cancels all animation frames, watchdog and fallback timers; resize, theme changes and page restoration never override a manual pause. Its timer fallback recovers visible Windows/RDP sessions when animation frames stall. The login sweep preserves its established behavior independently of Windows' reduced-motion signal; the pause control provides an explicit static alternative, while other UI motion still honors reduced-motion preferences.
...
- **Don't** introduce continuous decorative animation without user control, or colored glow shadows.
